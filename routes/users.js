    ...
    if (
      ...
      if (email.status && phone.status) {
        ...
        client.verify
          ...
          .then((verification) =>
            res.render("user/otp", { name_body: req.body, headers })
          );
      } else {
        // console.log('error')
      ...
      //console.log('error')
      res.render("user/signup", {
        err_msg: "Enter Every field Properly",
        headers,
      });
    }
  } catch {
...
router.get("/cart", verifyLogin, cartcnt, async (req, res) => {
  try {
    ...
    if (!products[0]) {
      ...
      totalamtwithotcoupon = await userHelper.getTotalAmountWithoutCoupon(
...
router.post("/Edit-Address/:addreessId", cartcnt, verifyLogin, (req, res) => {
  try {
    userHelper
      .updateAddress(req.params.addreessId, req.body)
      .then((response) => {
        res.redirect("/address");
      });
  } catch {
...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      userHelper
        .changeProductQuantity(req.body, productcount)
        .then(async (response) => {
...
router.post("/place-order", verifyLogin, async (req, res) => {
  try {
    ...
    if (req.body.paymentMethod == "COD") {
      // console.log(req.body)//return new Promise((resolve, reject) => {
      userHelper
        .placeOrder(req.body, products, totalPrice, walletupdate, wall)
        .then((response) => {
          res.json({ codsuccess: true });
        });
    } else if (req.body.paymentMethod == "razorpay") {
      userHelper
        .generateRazorpay(orderId, totalPrice, walletupdate, wall)
        .then((response) => {
      ...
      userHelper
        .generatePaypal(products, totalPrice, req.body, walletupdate, wall)
        .then((response) => {
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // console.log(order)
      userHelper
        .placeOrderpaypal(order, products, totalPrice, walletupdate, wall)
        .then((response) => {
          // res.json({codsuccess: true})
...
router.get("/orderdetail/:id", cartcnt, verifyLogin, async (req, res) => {
  try {
    cartCount = req.session.cartCount;
    let orders = await userHelper.getOrderDetail(
      req.params.id,
      req.session.user._id
    );
    headers = await userHelper.getHeader();
...
router.post("/orderpdf", cartcnt, verifyLogin, async (req, res) => {
  try {
    cartCount = req.session.cartCount;
    await userHelper
      .getOrderDetail(req.body.userid, req.session.user._id)
      .then((response) => {
        res.json(response);
      });
  } catch {
...
router.post("/edit-profile/:pid", cartcnt, verifyLogin, (req, res) => {
  try {
    ...
    userHelper
      .updateProfile(req.params.pid, req.body)
      .then(async (response) => {
        // cartCount=req.session.cartCount
        // let address=await userHelper.getAddress(req.session.user._id)
        // let user=await userHelper.getUsertDetails(req.session.user._id)
...
router.post("/changepassword", cartcnt, verifyLogin, async (req, res) => {
  try {
    ...
    headers = await userHelper.getHeader();
    userHelper
      .chekChangePassword(req.session.user, req.body)
      .then((response) => {
        if (response.status) {
          client.verify
            .services(serviceid)
            ...
            .then((verification) =>
              res.render("user/otp-changepass", {
                name_body: req.body,
                headers,
              })
            );
          ...
          res.render("user/change-password", {
            err_msg: "Enter The Correct Data",
            headers,
          });
        }
      })
      .catch((error) => {
        res.render("user/change-password", {
          err_msg: "Please Retry after sometimes",
          headers,
        });
      });
  } catch {
...
router.post("/changepass", cartcnt, verifyLogin, (req, res) => {
  try {
    client.verify
      ...
      .then((verification_check) => {
        userHelper
          .changepass(req.body, req.session.user._id)
          .then(async (response) => {
...
router.post("/verify-payment", verifyLogin, cartcnt, async (req, res) => {
  ...
  try {
    ...
    let products = await userHelper.getCartProductList(
      req.body.order.data.userId
    );
    let total_Price = await userHelper.getTotalAmount(
      req.body.order.data.userId
    );
    ...
    userHelper
      ...
      .then(() => {
        ...
        userHelper
          .placeOrderrazor(data, products, totalPrice, walletupdate, wall)
          .then((response) => {
...
router.post("/applycoupon", cartcnt, verifyLogin, async (req, res) => {
...
router.post("/removeCoupon", cartcnt, verifyLogin, (req, res) => {
 
  try {
    userHelpers
      .removeCoupon(req.body.userid, req.body.coupid)
      .then(async (response) => {
        userHelpers
          .removecoupnid(req.body.userid, req.body.coupid)
          .then(async (response) => {
            res.json({ status: true });
          });
      });
...
router.post("/wallet", cartcnt, verifyLogin, async (req, res) => {
  try {
    userHelpers.walletApply(req.body).then((response) => {
...
router.post("/walletremove", cartcnt, verifyLogin, (req, res) => {
  try {
    userHelpers.walletremove(req.body).then((response) => {
